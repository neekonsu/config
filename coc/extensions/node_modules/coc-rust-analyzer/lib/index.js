...
function createClient(bin, extra) {
  ...
  const standalone = import_coc.workspace.workspaceFolder ? !(0, import_fs.existsSync)((0, import_path.join)(import_coc.Uri.parse(import_coc.workspace.workspaceFolder.uri).fsPath, "Cargo.toml")) : true;
  if (standalone) {
...
function viewCrateGraph2(ctx) {
  return async () => {
    const svg = await ctx.client.sendRequest(viewCrateGraph, { full: false });
    import_coc6.window.echoLines([svg]);
  };
}
function viewFullCrateGraph(ctx) {
  return async () => {
    const svg = await ctx.client.sendRequest(viewCrateGraph, { full: true });
    import_coc6.window.echoLines([svg]);
  };
}
...
async function activate(context) {
  ...
  if (!(0, import_fs4.existsSync)(serverRoot)) {
    (0, import_fs4.mkdirSync)(serverRoot);
  }
  const bin = ctx.resolveBin();
  if (!bin) {
